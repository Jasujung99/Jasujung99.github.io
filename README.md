...
## 푸터 약관/개인정보 모달
- 푸터에 ‘서비스 이용 약관’과 ‘개인정보 처리 방침’ 링크가 밑줄로 표시됩니다. 클릭 시 반응형 모달(모바일 전면, 데스크톱 중앙 카드)로 원문을 그대로 보여줍니다.
- 접근성: ESC/오버레이 클릭/닫기 버튼으로 닫기, 포커스 트랩, 열릴 때 스크롤 잠금 적용.

구성 파일
- `src/legal/texts.ts` — 약관/개인정보 원문 텍스트(줄바꿈 유지)
- `src/components/Modal.tsx` — 접근성 모달 컴포넌트(반응형)
- `src/components/LegalModals.tsx` — 콘텐츠 렌더러(linkify 포함)
- `src/pages/HaeumHomePage.tsx` — 푸터 링크 및 모달 연결

---

## 교육·시험 상세 페이지 & 리드 폼(상담/문의)

- 새 페이지: `src/pages/EducationExam.tsx`
  - 한/영 병기: 소개 요약 → 대상(Target Audience) → 강점(Strengths & Features) → 프로그램/시험 안내 요약
  - 상·하단 CTA 버튼: [네이버 톡톡] [전화(tel:)] [상담 문의]
  - 페이지 내 상담 문의 섹션에서 `LeadForm` 사용
- 라우팅 연동: `src/App.tsx`에 `#/education-exam` 라우트 추가
- 홈 연동: `HaeumHomePage`의 프로그램 섹션에 “자세히 보기” 버튼 추가 → 상세 페이지로 이동
- 홈 문의 섹션 강화: 탭 구성(‘소식 받기’ | ‘상담/문의’)으로 전환, ‘상담/문의’ 탭에서 `LeadForm` 렌더

리드 폼 백엔드(Cloudflare Worker) 가이드
- 엔드포인트: `POST /api/lead`
- 요청 바디(JSON): `{ name, contact, inquiry, note, source }`
- Notion DB 권장 속성: Name(title), Contact(rich text), Inquiry(text), Note(rich text), Created(date)
- 환경 변수: `NOTION_TOKEN`, `NOTION_DATABASE_ID`, `ALLOWED_ORIGIN`
- CORS + 간단 레이트리밋 권장(아이피/세션 일 N회)

수정/교체 방법
- 법무 검토로 문구가 바뀌면 `src/legal/texts.ts`만 변경 후 배포하면 됩니다.
